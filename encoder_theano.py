...
def corpusConstructing(dirname, maxlen, tag='itself',p1=0.1,p2=0.1, **params):
    ...
    index_corpus = [[word2idx[word] if word in word2idx else -1 for word in sent] for sent in corpus]
...
def modelBuiliding(x_train, y_train, steps_per_epoch=30,epochs=3, tag = 'itself', extraEmbed = True,
                    max_features=20000,d=300,u=300,da=350,maxlen=22,r=30,dicsize=1000, **params):
    '''
    building model
    itype:
